...
def motor(tau, clock_sub, vel_sub, prev_vel):
    ...
    v = odeint(motor_model, prev_vel, t, args=(tau[0], tau[1]))
    clock_sub.prev_clock = clock_sub.clock
    #print('v = ', vel_sub.vel2)
    return v[9]


def motor_lsim(sys, tau , clock_sub, vel_sub, tau_dist):
    rclpy.spin_once(clock_sub)
    rclpy.spin_once(vel_sub)
    ...
    t, v, x = lsim(sys, [tau + tau_dist, tau + tau_dist], [0, clock_sub.clock - clock_sub.prev_clock], vel_sub.vel2)
    clock_sub.prev_clock = clock_sub.clock
    #print('v = ', vel_sub.vel2)
    return v[1]
...
def dist_motor_model(v, t, tau_left, tau_right):
    tau = [tau_left, tau_right]
    dv_dt = np.dot(A, v) + np.dot(B, tau) + [-0.01 * 4, -0.03 * 3]
    return dv_dt
